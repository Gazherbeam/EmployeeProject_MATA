...
public abstract class Employee implements Cloneable {
	...
	protected Employee(int empID, String empName, MyDate birthDate, MyDate hireDate) {
		this(empID, Name.fromString(empName), birthDate, hireDate);
	}

	protected Employee(int empID, Name empName, MyDate birthDate, MyDate hireDate) {
		this.setEmpID(empID);
	...
	public double computeTotalPayout(int targetMonth) {
		return this.computeSalary() + this.getBirthdayBonus(targetMonth);
	}
	...
	protected String employeeDetails() {
		return String.format("ID: %d, Name: %s, DOB: %s, Hired: %s",
			this.empID, this.empName, this.birthDate.toDisplayString(), this.hireDate.toDisplayString());
	}
}
